...
public class ControladorP implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == EjemploGUI.buttonUpload) {
			...
			if (edad<=0) {
				JOptionPane.showMessageDialog(null, "Error, Edad no valida: " + JOptionPane.ERROR_MESSAGE);
				errorT++;
			}
			
			if (pss<=60 || pss>240) {
				JOptionPane.showMessageDialog(null, "Error,  Presion Sistolica no valida(Fuera de rango) " + JOptionPane.ERROR_MESSAGE);
				errorT++;
			}
			if (hdl<40 || hdl>60) {
				JOptionPane.showMessageDialog(null, "Error, HDL no valido(fuera de rango) " + JOptionPane.ERROR_MESSAGE);
				errorT++;
			}

			if (chol<=5 || chol>200) {
				JOptionPane.showMessageDialog(null, "Error, Colesterol no valido(fuera de rango): " + JOptionPane.ERROR_MESSAGE);
				errorT++;
			}
